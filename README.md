...
```
ASR 引擎（Paraformer-zh，6 万小时中文训练）
  ↓ 原始识别
领域词库（70+ Crypto/AI 术语预置）
  ↓ 术语纠正
...
内置 Flashcard 式校准系统——朗读一段领域文本，系统自动对比识别结果和标准文本，找出你的发音特征并记录为纠正规则。针对你个人的发音习惯做优化，而不是泛泛的通用模型。
...
cd speech
bash Scripts/setup.sh                              # 下载框架 + 模型（~300MB，仅首次）
swift build && codesign --force --sign - .build/debug/AzexSpeech
.build/debug/AzexSpeech
```
...
| ASR | [sherpa-onnx](https://github.com/k2-fsa/sherpa-onnx) Paraformer-zh | 离线，int8 量化 ~217MB |
